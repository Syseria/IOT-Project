...
ALL_ERROR = (
    ...
    SensorStatus.CARGO_CREATE_ERROR |
    SensorStatus.CARGO_CALIBRATE_ERROR |
    SensorStatus.SENSOR_BUFFER_ERROR |
    SensorStatus.CARGO_BUFFER_ERROR |
    SensorStatus.CONFIG_APPLY_ERROR |
    SensorStatus.APPLICATION_ERROR |
...
# ===============================
class SensorResultHeader(IntFlag):
...
class A121_Cargo_Example:
    ...
    def get_cargo_application(self):
        self.set_command(SensorCommand.MEASURE_UTILIZATION_LEVEL)
        ...
        if header & SensorResultHeader.UTILIZATION_LEVEL_VALID:
